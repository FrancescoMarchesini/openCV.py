...
class MotionDetector:
    ...
    def detect(self, image, vicini, costante):
        #compute la differnza assuluta tra il bg model e il frame attuale
        delta = cv2.absdiff(self.bg.astype("uint8"), image)
        #passo il delta e faccio il threshold
        #thresh = cv2.threshold(delta, 25, 255, cv2.THRESH_BINARY)[1]
        #thresh = cv2.adaptiveThreshold(delta, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, vicini, costante)
